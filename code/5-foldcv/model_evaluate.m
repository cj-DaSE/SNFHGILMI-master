function result=model_evaluate(interaction_matrix,predict_matrix,train_ddi_matrix)%全部数据的邻接矩阵；模型预测的相互作用得分；训练样本集
    ...
    [sen,accuracy,f1]=evaluation_metric(real_score,predict_score);
    result=[auc,sen,accuracy,f1];
end


function [sen,accuracy,f1]=evaluation_metric(interaction_score,predict_score)
    max_value=max(predict_score);
    min_value=min(predict_score);
    threshold=min_value+(max_value-min_value)*(1:999)/1000;
    for i=1:999
       predict_label=(predict_score>threshold(i));%将得分转换为0，1值
